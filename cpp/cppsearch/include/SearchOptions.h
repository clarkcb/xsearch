...
namespace cppsearch {
    ...
    private:
        std::unordered_map<std::string, std::function<void(bool, SearchSettings&)>> m_bool_arg_map = {
            ...
            {"noprintmatches", [](const bool b, SearchSettings& ss) { ss.print_matches(!b); }},
            {"noprintresults", [](const bool b, SearchSettings& ss) { ss.print_results(!b); }},
            ...
            {"printmatches", [](const bool b, SearchSettings& ss) { ss.print_matches(b); }},
            {"printresults", [](const bool b, SearchSettings& ss) { ss.print_results(b); }},
