...
export class Searcher {
  ...
  private validateSettings(): void {
    try {
      ...
      assert.ok(
        this._supportedEncodings.indexOf(this._settings.textFileEncoding) > -1,
        'Invalid encoding',
      );
  ...
  public async search(): Promise<SearchResult[]> {
    ...
    const searchResultsArrays: SearchResult[][] = await Promise.all(
      fileResults.map((fr) => this.searchFile(fr)),
    );
  ...
  private async searchBinaryFile(fileResult: FileResult): Promise<SearchResult[]> {
    ...
    const contents: string = FileUtil.getFileContentsSync(
      fileResult.filePath,
      this._binaryEncoding,
    );
    let results: SearchResult[] = [];
    ...
    const patternResultArrays: SearchResult[][] = this._settings.searchPatterns.map((p: RegExp) =>
      searchPattern(p),
    );
  ...
  private async searchTextFileContents(fileResult: FileResult): Promise<SearchResult[]> {
    const contents: string = FileUtil.getFileContentsSync(
      fileResult.filePath,
      this._settings.textFileEncoding,
    );
  ...
  private static getLinesAtIndices(
    s: string,
    atIndices: number[],
  ...
  private static getLinesBefore(
    s: string,
    beforeStartIndices: number[],
    startLineIndices: number[],
    endLineIndices: number[],
  ): string[] {
    return Searcher.getLinesAtIndices(s, beforeStartIndices, startLineIndices, endLineIndices);
  }

  private static getLinesAfter(
    s: string,
    afterStartIndices: number[],
    startLineIndices: number[],
    endLineIndices: number[],
  ): string[] {
    return Searcher.getLinesAtIndices(s, afterStartIndices, startLineIndices, endLineIndices);
  }
    ...
    this._settings.searchPatterns.forEach((p: RegExp) => {
      ...
      while (match && !stop) {
        ...
        if (self._settings.linesBefore && beforeLineCount) {
          linesBefore = Searcher.getLinesBefore(
            s,
            beforeStartIndices,
            startLineIndices,
            endLineIndices,
        ...
        if (self._settings.linesAfter) {
          ...
          if (afterStartIndices.length > self._settings.linesAfter) {
            afterStartIndices = afterStartIndices.slice(0, self._settings.linesAfter);
          }
          linesAfter = Searcher.getLinesAfter(
            s,
            afterStartIndices,
            startLineIndices,
            endLineIndices,
        ...
        if (
          (self._settings.linesBefore === 0 || self.linesBeforeMatch(linesBefore)) &&
  ...
  private linesBeforeMatch(linesBefore: string[]): boolean {
    return Searcher.linesMatch(
      linesBefore,
      this._settings.inLinesBeforePatterns,
  ...
  private linesAfterMatch(linesAfter: string[]): boolean {
    return Searcher.linesMatch(
      linesAfter,
      this._settings.inLinesAfterPatterns,
  ...
  private async searchTextFileLines(fileResult: FileResult): Promise<SearchResult[]> {
    const self = this;
    const lines: string[] = FileUtil.getFileLinesSync(
      fileResult.filePath,
      this._settings.textFileEncoding,
    );
  ...
  public async searchLines(lines: string[]): Promise<SearchResult[]> {
    ...
    while (true) {
      ...
      if (this._settings.linesBefore > 0) {
        if (linesBefore.length == this._settings.linesBefore) linesBefore.shift();
        if (linesBefore.length < this._settings.linesBefore) linesBefore.push(line);
      }
    }
    return results;
  }
  ...
  printMatchingLines(results: SearchResult[], formatter: SearchResultFormatter): void {
    ...
    let hdrText: string;
    if (this._settings.uniqueLines) hdrText = '\nUnique matching lines';
    else hdrText = '\nMatching lines';
    if (lines.length > 0) {
  ...
  getMatches(results: SearchResult[]): string[] {
    let matches: string[] = results
  ...
  printMatches(results: SearchResult[], formatter: SearchResultFormatter): void {
    const matches: string[] = this.getMatches(results);
    let hdrText: string;
    if (this._settings.uniqueLines) hdrText = '\nUnique matches';
    else hdrText = '\nMatches';
    if (matches.length > 0) {
