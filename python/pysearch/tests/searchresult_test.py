...
class SearchResultTest(unittest.TestCase):
    ...
    def test_single_line_search_result(self):
        ...
        sf = SearchFile(path=self.cssearch_path, file_name=file_name, file_type=FileType.CODE)
        line_num = 10
        ...
        expectedoutput = "{0:s}: {1:d}: [{2:d}:{3:d}]: {4:s}".format(os.path.join(self.cssearch_path, file_name),
    ...
    def test_single_line_longer_than_max_length_search_result(self):
        ...
        sf = SearchFile(path='.', file_name=file_name, file_type=FileType.TEXT)
        line_num = 1
    ...
    def test_binary_file_search_result(self):
        ...
        sf = SearchFile(path=self.cssearch_path, file_name=file_name, file_type=FileType.BINARY)
        line_num = 0
        match_start_index = 0
        match_end_index = 0
        line = ''
        ...
        search_result = SearchResult(pattern=pattern,
                                     ...
                                     lines_before=lines_before,
                                     lines_after=lines_after)
        expectedoutput = "{0:s} matches at [{1:d}:{2:d}]".format(os.path.join(self.cssearch_path, file_name),
                                                                 match_start_index, match_end_index)
        output = formatter.format(search_result)
        self.assertEqual(expectedoutput, output)

    def test_multiline_search_result(self):
        formatter = SearchResultFormatter(SearchSettings(colorize=False))
        pattern = "Search"
        file_name = 'Searcher.cs'
        sf = SearchFile(path=self.cssearch_path, file_name=file_name, file_type=FileType.TEXT)
        line_num = 10
        match_start_index = 15
        match_end_index = 23
        line = "\tpublic class Searcher\n"
        ...
        search_result = SearchResult(pattern=pattern, file=sf,
