...
sub test_single_line_search_result {
    ...
    my $lines_before = [];
    my $lines_after = [];
    my $search_result = new plsearch::SearchResult($pattern, $file, $line_num, $match_start_index,
        $match_end_index, $line, $lines_before, $lines_after);
    my $expected_output = sprintf("%s: %d: [%d:%d]: %s", $file, $line_num,
...
sub test_binary_file_search_result {
    ...
    my $lines_before = [];
    my $lines_after = [];
    my $search_result = new plsearch::SearchResult($pattern, $file, $line_num,
        $match_start_index, $match_end_index, $line, $lines_before, $lines_after);
    my $expected_output = sprintf("%s matches at [%d:%d]", $file,
...
sub test_multi_line_search_result {
    ...
    my $expected_output = sprintf($outputtemplate, $file, $line_num,
        $match_start_index, $match_end_index);
    my $output = $formatter->format($search_result);
    ok($expected_output eq $output,``
