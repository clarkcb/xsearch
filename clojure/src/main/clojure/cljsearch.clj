...
(def DEFAULT-DIR-FILTER-PATTERNS  '(#"^\\.svn" #"^CVS$"))
(def DEFAULT-FILE-FILTER-PATTERNS '(#"^\\.DS_Store$"))
...
(defn usage []
  (println "Usage:")
  (println "search [options] -s <searchstring> <startdir>\n")
  (println "options:")
  (let [longest (longest-length (for [option OPTIONS] (get option :long)))
        format-string (str "%s --%-" (+ 3 longest) "s %s")] ; need $ somewhere
    (loop [options OPTIONS]
      (println
        (format format-string
          (if (get (first options) :short)
            (str "-" (get (first options) :short) " |")
            "    ")
          (if (get (first options) :arg)
            (str (get (first options) :long) " x")
            (get (first options) :long))
          (get (first options) :desc)))
      (if (second options)
        (recur
          (rest options))
        (System/exit 0)))))
